...
<body>
  <div class="card">
    ...
    <h2>3. Google Analytics and Event Tracking</h2>
    <p>We use Google Analytics (specifically Google Analytics 4) to collect information about how our users interact with the application. This helps us analyze app usage, improve functionality, monitor performance, and provide a better overall experience.</p>
    <p>The information collected via Google Analytics is aggregated, anonymous, and includes:</p>
    <ul>
      <li>Basic usage statistics (such as page views, button clicks, and screen transitions).</li>
      <li>Custom application events (such as tracking when a user chats with the AI assistant, scans a medication label, checks for drug interactions, imports/exports data, or logs intake schedules).</li>
      <li>Technical metrics (such as device type, browser, and general geographic location derived from IP addresses).</li>
    </ul>
    <p>We do not send sensitive personal health information, E2EE keys, passwords, or medication imagery to Google Analytics. All tracking is used solely for service optimization and user engagement metrics.</p>

    <h2>4. Compliance and Contact</h2>
